...
function authenticate(req, res, next){
    ...
    pg.connect(process.env.DB_URI, function(err, client, done){ // if conString doesn't work, replace with process.env.DB_URI
        if(err) {
        ...
        client.query('SELECT * FROM users WHERE email=$1', [body.email], function(err, user){
            if(!bcrypt.compareSync(req.body.password, user.rows[0].passworddigest)){
                // if it doesn't...
